...
object TestAuth extends Authentication with ComWrapper:
  ...
  def testBasic_sha256(group: String, number: Int, param: String): Future[Either[Error, String]] =   
    sha256(param).map { 
      case Left(error)  =>
  ...
  def testBasic_checkPasswordFormat(group: String, number: Int, param: String): Future[Either[Error, String]] =                   
  ...
  def testBasic_regUser(group: String, number: Int, param: String): Future[Either[Error, String]] =
    val email = param
    val password = "Abc1234567"

    (for
      pw   <- EitherT[Future,Error,String](sha256(password))
      usr  <- EitherT[Future,Error,User](regUser(User("Robert", "Lichtenegger", email), pw))
    yield usr).value.map {
  ...
  def testBasic_login(group: String, number: Int, param: String): Future[Either[Error, String]] =
  ...
  def testBasic_verifyUser(group: String, number: Int, param: String): Future[Either[Error, String]] =
    ...
    postJson[Int]("/test/auth/setUserVerify", List(("id", id.toString),("value", value.toString))).map {
      case Left(err)  => addOutput(s"ERROR: ${err}");     Left(err)
  ...
  def testBasic_getVerifyLink(group: String, number: Int, param: String): Future[Either[Error, String]] =
    var email = ""
    var id    = 0L
    ...
    getData("/test/auth/getUserVerifyLink", List(("email", email),("id",id.toString))).map {
      case Left(err) => addOutput(s"ERROR: ${err}");        Left(err)
  ...
  def testBasic_getUserInfo(group: String, number: Int, param: String): Future[Either[Error, String]] =
    var email = ""
    var id    = 0L
    val iTyp = param.split(";")(0)
    val value  = param.split(";")(1)
    ...
    getJson[User]("/test/auth/getUserInfo", List(("email", email),("id",id.toString))).map {
      case Left(err)   => addOutput(s"ERROR: ${err}");     Left(err)
      case Right(user) => addOutput(s"RESULT: user->${user}"); Right(s"FINISHED: ${group}-Test:${number} param:${param}")
    }
  ...
  def testBasic_setUserPassword(group: String, number: Int, param: String): Future[Either[Error, String]] =
  ...
  def testBasic_checkUserAuth(group: String, number: Int, param: String): Future[Either[Error, String]] =
    val NAME = "testBasic_checkUserAuth"

    val (email,password) = param.toTuple(";")
    getData("/test/auth/checkUserAuth", List()).map {
      case Left(err)   => addOutput(s"ERROR: ${err}");       Left(err)
      case Right(res)  => addOutput(s"RESULT: res->${res}"); Right(s"FINISHED ${NAME}: ${group}-Test:${number} param:${param}")
    }
