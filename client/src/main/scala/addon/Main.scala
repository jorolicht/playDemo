...
object Addon extends JsWrapper: 
  ...
  def execute(cmd: String): Future[Either[Error, String]] =    
    ...
    args(0).toLowerCase match
      ...
      case _       => 
        ...
        Future(Left(Error("Invalid command"))) 


  /** log command
   * 
   */ 
  def cmdLog(args: Array[String]): Future[Either[Error, String]] = {
    ...
    try
      ...
      level match 
        ...
        case _       => Future(Left(Error("command.invalid"))) 
    catch { case _:Exception => Future(Left(Error("command.invalid"))) } 
   ...
   * 
   */ 
  def cmdTest(args: Array[String]): Future[Either[Error, String]] = 
