...
trait Authentication extends ComWrapper: 
  ...
  def sha256(text: String): FuEiErr[String]  = 
    js.Dynamic.global.sha256(text.asInstanceOf[js.Any]).asInstanceOf[js.Promise[String]]
      .toFuture
      .map { res => Right(res) }
      .recover { case error => Left(Error("", error)) }
  ...
  def basicLogin(email: String, password: String): FuEiErr[User] =
    ...
    def doBasicLogin(authCode: String):FuEiErr[User] =
      ...
      Ajax.get(path, headers = Map("Authorization"-> authCode)).map(_.responseText).map(user => {
        ...
        case _: Throwable               => Left(Error("err00003.ajax.login", "noResponse", "noStatus"))
      })

    (for 
      pwEnc <- EitherT[Future,Error,String](sha256(password))
      usr   <- EitherT[Future,Error,User](doBasicLogin("Basic " + utob(s"${email}:${pwEnc}")))
    yield usr).value
  ...
  def regUser(user: User, password: String): FuEiErr[User] = 
    (for 
      pwEnc <- EitherT[Future,Error,String](sha256(password))
      usr   <- EitherT[Future,Error,User]( postJson[User]("/auth/regUser", List(), toJson(user.copy(password=pwEnc))) )   
    yield usr).value  
  ...
  def logout(user: User): FuEiErr[Boolean] = 
    postJson[Boolean]("/auth/logout", List(), toJson(user))
  ...
  def setUserPassword(email: String, password: String): FuEiErr[Int] = 
    (for 
      pwEnc <- EitherT[Future,Error,String](sha256(password))
      res   <- EitherT[Future,Error,Int]( postJson[Int]("/auth/setUserPassword", List(("email", email)), pwEnc) )
    yield res).value
  ...
  def setUserVerify(email: String, value: Boolean=true): FuEiErr[Int] = 
    postJson[Int]("/auth/setUserVerify", List(("value", value.toString)), email)
