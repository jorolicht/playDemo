...
trait Encryption:
  ...
  def getUserFromCookie[A](request: play.api.mvc.Request[A])(using decoder: Base64.Decoder): Either[shared.model.Error, User] =
    val curUnixTime: Long = Instant.now().getEpochSecond
    request.cookies.get(UserAuthCookie) match
      case None         => Left(shared.model.Error("NoCookie"))
      case Some(cookie) => 
        try
          val (usr, cookieTime) = read[(User,Long)](decode64(cookie.value))
          if ((curUnixTime - cookieTime ) < CookieValidSec) then Right(usr) else Left(shared.model.Error("CookieTimeout"))
        catch
          case e:Exception => Left(shared.model.Error("CookieDecode"))
