...
class Auth @Inject()(cc: ControllerComponents, mailer: MailerClient, cfg: Configuration, userRepo: UserRepository)
    ...
    if (idToken != null) then
      ...
      userRepo.getUser(email).flatMap( _ match {
        ...
        case Right(usr) => if (usr.lastname.toLowerCase == lastname.toLowerCase) 
          ...
          else Future(BadRequest(toJson(Error("err00013.auth.google",usr.lastname, lastname))).discardingCookies(genDiscardAuthCookie)) 
      })
    else 
      Future(BadRequest(toJson(Error("err00014.auth.token.invalid"))))
  }
  ...
  def basicLogin(): Action[AnyContent] = Action.async { implicit request =>
    ...
    catch case e: Throwable => Future(BadRequest(toJson(Error("err00006.parseJson"))))
  }
  ...
  def verifyUser(code: String): Action[AnyContent] = Action.async { implicit request =>
    ...
    parseJson[(Long,String,Long)](decode64(code)) match 
      ...
      case Right(vCode) => userRepo.setEmailVerified(vCode._1).map {
        case Left(err)  => Ok(views.html.main("Error", encode64(toJson(Error("verifyUserNotPossible"))), msgsB64))
        case Right(res) => if (res) 
          then Ok(views.html.main("Home", "verified", msgsB64)) 
          else Ok(views.html.main("Error", encode64(toJson(Error("verifyUserNotPossible"))), msgsB64))
      }
  }     


  /** request to register a user return generated user (with id) else Error
