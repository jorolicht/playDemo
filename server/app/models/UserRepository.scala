...
class UserRepository @Inject()(dbapi: DBApi)(implicit ec: DatabaseExecutionContext) {
  ...
  def insert(user: User): FuEiErr[User] = Future {
    ...
    try
      (db.withConnection { implicit connection =>
        ...
        case None      => Left(shared.model.Error("err00008.db.user.insert", "no id")) 
    catch { case e: Exception => Left(shared.model.Error("err00008.db.user.insert", e.getMessage)) }
  }(ec)
  ...
  def setPassword(email: String, password: String): FuEiErr[Int] = Future {
    try
      ...
      Right(result) 
    catch { case e: Exception => Left(shared.model.Error("err00007.db.user.update", e.getMessage)) }
  }(ec)  
  ...
  def verify(email: String, password: String): FuEiErr[User] = Future {
    ...
    try
      (db.withConnection { implicit connection =>
        ...
        case Some(usr) => if (usr.password == password) Right(usr) else Left(Error("err00015.login.invalid"))
        case None      => Left(Error("err00015.login.invalid"))
    catch { case e: Exception => Left(Error("err00011.db.user.select", e.getMessage)) }
  }(ec)
  ...
  def getUser(email: String="", id: Long=0L): FuEiErr[User] = Future {
    try
      (db.withConnection { implicit connection =>
        ...
        case None       => Left(shared.model.Error("err00010.db.user.read", s"${email}${id}"))
      } 
    catch { case e: Exception => Left(shared.model.Error("err00011.db.user.select", e.getMessage)) }
  }(ec)    
